...
class MyApp(QWidget):
    ...
    def LoadPlates(self):
        ...
        if self.objectPlates.text():
            ...
            self.plateCatalogue, wrongPlates = func.CatalogueThinning(rawCatalog)
    ...
    def screenPlans(self):
        ...
        btnUploadToMxlive.clicked.connect(self.zero)
        ...
        grid.addWidget(btnUploadToMxlive,         6,0,1,10)
        ...
        try:
            ...
            if event.buttons() & Qt.LeftButton:
                if self.eraserFlag:
                    self.distanToPoint(xPixcelPosition,yPixcelPosition)
                    text = 'Remove a point: {0} point(s) in {1}@{2}'.format(len(self.targetPoints[imgKey]),well,pID)
                    #print(type(self.targetPoints[imgKey]))
                    #print(len(self.targetPoints[imgKey]))
                    #if len(self.targetPoints[imgKey]) == 0:
                    #    del self.targetPoints[imgKey]
                    #    print('deselected')
                else:
                    self.scale.append([round(xMeterPosition,0),round(yMeterPosition,0), xPixcelPosition, yPixcelPosition])
                    if len(self.scale) == 1:

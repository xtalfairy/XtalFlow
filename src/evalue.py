...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        #print(index.__init__.test)
        #print(index.__init__.message)
        ...
        self.pathway     = {'rmserver':'/smbmount/rmserver/RockMakerStorage/WellImages',\
                            ...
                            'userhome':'/data/users/{0}'.format(misc.getUsername())}
        ...
        leftgrid.addWidget(self.ObjectPlates(),  1,0)
        ...
        rightgrid.addWidget(self.requestBox(),     2,0)
    ...
    def LoadPlates(self):
        ...
        if self.objectPlates.text():
            ...
            for item in rawList:
                pID = item.strip()
                #rawCatalog[pID] = func.checkPlates(pID, imageProfile, self.targetPoints, self.pathway['rmserver'],  self.cellarPath)
                self.plateCatalogue[pID] = func.checkPlates(pID, imageProfile, self.targetPoints, self.pathway['rmserver'],  self.cellarPath)
            #rawDict = self.plateCatalogue
            plateCatalogue, wrongPlates = func.CatalogueThinning(self.plateCatalogue)
            self.plateCatalogue = plateCatalogue
            #self.plateCatalogye = {'690': {'plate': 'present', 'profile': 'profileID_1', \
            #                               'image': 'present', 'drops': 288, \
            #                               'rank': 'none', 'selected': 0, 'warning': 'none'}, ... }
            self.refreshPlates()
    ...
    def setCurrentPlate(self):
        ...
        currPlate_infoPath = "{0}/{2}_pID{1}_info.csv".format(self.cellarPath, self.currentPlate.text(), misc.getUsername())
    ...
    def imgNavigator(self):
        ...
        undoButton.clicked.connect(self.removePoint)
        ...
        grid.addWidget(buttonWidget,             2,0,1,4)
    ...
    def moveToPlate(self):
        ...
        if wannaGo in plates:
            currPlate_infoPath = "{0}/{2}_pID{1}_info.csv".format(self.cellarPath, wannaGo, misc.getUsername())
            self.currPlate_info = misc.csvToList(currPlate_infoPath)
    ...
    def moveNextWell(self):
        ...
        return 0
    """
    def moveNearbyWell(self):
        #self.homeButton = QPushButton()
        #self.lastButton = QPushButton()

        pID = self.currentPlate.text()
        lastWellNo = len( self.currPlate_info )-1 #int(1) for headerline

        sender = self.mainWidget.sender()
        if self.nextButton == sender:
            step = 1
            limit = lastWellNo
            message = '[moveToWell] Last Well in This Plate'
        elif self.prevButton == sender:
            step = -1
            limit = 1
            message = '[MoveToWell] First Well in This Plate'
        else: pass #I cannot sure this else is necessary

        if pID != 'none' or pID != '':
            if self.currWellNo == limit:
                print(message)
            else:
                self.currWellNo += step
                self.setCurrentWell()
                print('[moveToWell] {0} >> {1}'.format( self.currPlate_info[self.currWellNo-step][2], self.currPlate_info[self.currWellNo][2] ))
        else: pass
        return 0
    """
    ...
    def screenPlans(self):
        ...
        btnUploadToMxlive = QPushButton("Upload To MxLive")
        btnUploadToMxlive.setIcon(self.style().standardIcon(getattr(QStyle, 'SP_ArrowUp')))
        btnUploadToMxlive.clicked.connect(self.deliverToMxlive)
        ...
        grid.addWidget(btnUploadToMxlive,         8,0,1,10)
    ...
    def cryoPlans(self):
        ...
        sites = ['To A Point', 'To 4 Sides', 'To 6 Sides', 'Assign Manually']
        for item in sites:
        ...
        grid.addWidget(QLabel("Targets/Well"),    1,0,1,4)
        grid.addWidget(self.cryoSites,            1,4,1,6)
        grid.addWidget(QLabel("Volume/Well"),     2,0,1,4)
        grid.addWidget(self.cryoVolume,           2,4,1,6)
        grid.addWidget(QLabel("Source Well"),     3,0,1,4)
        grid.addWidget(QLineEdit(),               3,4,1,6)
    ...
    def selectLibrary(self):
        ...
        libfile = "{0}/{1}.csv".format(self.libraryPath,library)
        self.libraryInfo = []
    ...
    def checkScreenPlans(self):
        #ECHO650/SHIFTERs connection
        #Is it given? : protein name / targetsites / library / region(default=full) / volume 
        return 0
    ...
    def makeEvalWorksheet(self):
        ...
        doCryo = []
        durationVolume = {}
        for item in solvents+['CRYO']:
            durationVolume[item] = 0
        print(solvents)
        print(times)
        print(solvVol, cryoVol)
                    ...
                    for sv in solvVol:
                        ...
                        if self.replica.value() == 1:
                            ...
                            for i in range(int(self.replica.value())):
                                ...
                                #if cDo: doCryo.append(cDo)
        for i,item in enumerate(appendixes):
            try:
                print(item, doSolvent[i], doCryo[i])
            except IndexError: pass
        self.requestMessage.append('Select {0} drops from the plates'.format(len(appendixes)))
        ...
        for key,val in self.targetPoints.items():
            try:
                ...
                for i,item in enumerate(val):
                    ...
                    if doCryo:
                        ctimes = float(doCryo[cnt][2]) / 2.5 # 2.5ul : Minimum transfer value
                        ...
                        echoCryoList.append( [doCryo[cnt][0], srcPlate, doCryo[cnt][1], ctransVol,\
        ...
        try:
            ...
            if len(echoCryoList) > 1:
                misc.listToCsv(echoPath, EvalueExpID+'_Cryo', echoSolvList)
    ...
    def makeChemWorksheet(self):
        echoHeader = ['Source plate name', 'Source Well','Transfer Volume'\
                      'Destination plate name', 'Targeting Well(forView)','Destination Well',\
                      'Destination Well X offset', 'Destination Well Y offset']
        #match TargetList & LibraryList
        #How can we devide the drop volume?
        ...
        for key,val in self.targetPoints.items():
            try:
                ...
                for i,item in enumerate(val):
                    ...
                    #Assign Transfer volume to each value(points)
                    times = self.targetVol.value() / 2.5 # 2.5ul : Minimum transfer value
                    quota = times // len(val)
                    ...
                    if i+1 == len(val):
                        ...
                        transVol = quota * 2.5
                    targetList.append( [srcPlate, srcWell, transVol,destPlate, destWell1, destWell2, destXoffset, destYoffset] )
                cnt += 1
            except IndexError as e:
                pass
        for item in targetList:
            print('[saveWorkSheet]',item)
        print(chosenWell)
        print('count well', len(chosenWell))
        return targetList
    ...
    def imageClickEvent(self, event):
        ...
        try:
            ...
            if event.button() & Qt.RightButton:
                ...
                """
                try:
                    if len(self.Targeting[imgKey]) < self.divisions.value():
                    ...
                    if targetsInThisWell == self.divisions.value():
                        print(targetsInThisWell ,self.divisions.value())
                        self.moveNextWell()
                    else: pass
                """

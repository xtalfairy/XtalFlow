...
def checkPlates(pID, imgProfile, Targeting, StoragePath, cellarPath):
    ...
    survey = { 'plate':'missing', 'profile':imgProfile, 'image':'missing', \
               ...
               'warning':'none'}
    if not os.path.isdir(cellarPath):
        misc.createFolder(cellarPath)
    else: pass
    infoChart = '{0}/{2}_pID{1}_info.csv'.format(cellarPath, pID, misc.getUsername())
    rankChart = '{0}/{2}_pID{1}_rank.csv'.format(cellarPath, pID, misc.getUsername())

    try:
        realPlates = sorted(os.listdir(StoragePath))
        print('[Load Plate] connect to RMServer: {0}'.format(StoragePath))
        #20221115 correspond to 0000
        if pID in realPlates or pID[2:] in realPlates or pID[1:] in realPlates:
        #if pID in realPlates:
            print('iamhere')
            #number = [n(row), n(well), n(drop)]
            number = {'wells':0, 'drops':0}
            pIDpath     = "{0}/{1}/plateID_{1}".format(StoragePath, pID)
            #20221114
            #if int(pID) > 999 and int(pID) < 2000: pIDpath = "{0}/{1}/plateID_{2}".format(StoragePath, pID[2:], pID)
            #20221205
            if int(pID) > 999 and int(pID) < 1100: pIDpath = "{0}/{1}/plateID_{2}".format(StoragePath, pID[2:], pID)
            elif int(pID) >= 1100 and int(pID) < 2000: pIDpath = "{0}/{1}/plateID_{2}".format(StoragePath, pID[1:], pID)
            else: pIDpath = "{0}/{1}/plateID_{2}".format(StoragePath, pID, pID)
            ...
            for row in plateGrid['row']:
                for col in plateGrid['col']:
                    number['wells'] += 1
                    rmsWell = "wellNum_{0}".format(number['wells'])
                    wellPath = "{0}/{1}/{2}".format(pIDlatePath, rmsWell, imgProfile)
                    #print('[Load Plate] Current Well: {0}'.format(wellPath))
                    if os.path.isdir(wellPath):
                        for i, drop in enumerate(plateGrid['baby']):
                            subwell = i+1
                            dropNo = "d{0}".format(i+1)
                            uniWell = "{0}{1:02d}{2}".format(row, col, drop)
                            try:
                                imgName = [item for item in sorted(os.listdir(wellPath)) if item.startswith(dropNo) and item.endswith('ef.jpg')][-1]
                                imgPath = "{0}/{1}".format(wellPath, imgName)
                                if os.path.isfile(imgPath):
                                    number['drops'] += 1
                                    if number['drops'] == 1:
                                        with open(infoChart, 'w') as f:
                                            writer = csv.writer(f)
                                            writer.writerow(['nu', 'plateID', 'Well', 'WellNo', 'Subwell', 'ImgCreaged', 'IMGpath'])
                                    else: pass
                                    imgCreated = datetime.fromtimestamp( os.path.getctime(imgPath) )
                                    #print(uniWell, imgPath, dropNo)
                                    with open(infoChart, 'a') as f:
                                        writer = csv.writer(f)
                                        writer.writerow([number['drops'], pID, uniWell, rmsWell, subwell, imgCreated, imgPath])
                                    survey['image'] = 'present'
                                else: pass
                            except FileNotFoundError as errMessage:
                                #print(errMessage)
                                print('[Load Plate] [!] pID{0}_{1}: No {2} Image Found'.format(pID,uniWell, imgProfile))
                            except IndexError as errMessage:
                                print('[Load Plate] [!] pID{0}_{1}: No {2} Image Found'.format(pID,uniWell, imgProfile))
                ...
                if number['drops'] != 0:
                    ...
                    survey['warning'] = 'pID{0}: No {1} image'.format(pID,imgProfile)
                if len(Targeting) != 0:
                    cnt = 0
                    #print('[Load Plate]', pID, Targeting)
                    for key,val in Targeting.items():
                        if key.startswith(pID):
                            print(key)
                            cnt += 1
                    survey['selected'] = cnt
                    #print('[Load Plate] pID{0} has {0} Target(s)'.format(pID, survey['selected']))
                else:
                    pass
                if os.path.isfile(rankChart):
                    survey['rank'] = 'done'
                else:
                    pass

        else:
            survey['warning'] = 'pID{0}: Absence'

    except FileNotFoundError as e:
        print('[Load Plate] [!] Unable to connect to RockMakerServer')
